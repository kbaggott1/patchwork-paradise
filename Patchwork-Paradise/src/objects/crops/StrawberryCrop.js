...
export default class StrawberryCrop extends Crop {
    static TOTAL_GROW_TIME = 10
    static SPRITE_START = 12
    ...
    initSprites() {
        ...
        return sprites.splice(StrawberryCrop.SPRITE_START, Crop.GROW_STATES)
    }
    ...
    update(dt) {
        ...
        if(!this.isGrowing) {
            this.isGrowing = true;
            timer.wait(StrawberryCrop.TOTAL_GROW_TIME / Crop.GROW_STATES - 1, () => {
                this.currentFrame++;
                this.isGrowing = false;
                this.offset = new Vector(2, -4)
            }) 
        }
    ...
    harvest() {
        return ItemFactory.CreateInstance(ItemType.Strawberry)
    }

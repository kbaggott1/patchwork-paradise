...
export default class PotatoCrop extends Crop {
    static TOTAL_GROW_TIME = 10
    static SPRITE_START = 30
    ...
    initSprites() {
        ...
        return sprites.splice(PotatoCrop.SPRITE_START, Crop.GROW_STATES)
    }
    ...
    update(dt) {
        ...
        if(!this.isGrowing) {
            this.isGrowing = true;
            timer.wait(PotatoCrop.TOTAL_GROW_TIME / Crop.GROW_STATES - 1, () => {
                this.currentFrame++;
                this.isGrowing = false;
    ...
    harvest() {
        return ItemFactory.CreateInstance(ItemType.Potato)
    }
}